...
function image(p){
  const i=String(p.image||p.images?.[0]||'').trim();if(!i)return'✨';
...
function renderProductPage(){
  ...
  renderMainProductImage();renderThumbs();renderSimilarProducts(p,key);setupLightboxGestures();
}
...
function setupLightboxGestures(){
  const stage=document.getElementById('lightboxStage');if(!stage||stage.dataset.ready)return;stage.dataset.ready='1';
  stage.addEventListener('pointerdown',e=>{if(lightboxScale<=1)return;dragStart={x:e.clientX-lightboxPanX,y:e.clientY-lightboxPanY};stage.setPointerCapture(e.pointerId)});
  stage.addEventListener('pointermove',e=>{if(!dragStart||lightboxScale<=1)return;lightboxPanX=e.clientX-dragStart.x;lightboxPanY=e.clientY-dragStart.y;applyTransform()});
  stage.addEventListener('pointerup',()=>dragStart=null);stage.addEventListener('pointercancel',()=>dragStart=null);
}
