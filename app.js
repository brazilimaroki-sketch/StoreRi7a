...
function finalPrice(p){
  return p.sale_price!==null &&
         Number.isFinite(Number(p.sale_price)) &&
...
function discountPercent(p){
  if(Number(p.discount)>0){
    return Math.round(Number(p.discount));
  }
...
function renderCurrentPage(){
  /* مهم: product.html فيه productView و loadingBox، ماشي productDetails */
...
function renderProductPage(){
  ...
  if(!p){
    ...
    if(error){
      error.hidden=false;
      error.innerHTML='المنتج غير موجود أو تعذر تحميله.<br><a href="/">العودة للرئيسية</a>';
    }
    return;
  }
  ...
  if(currentPriceEl){
    currentPriceEl.innerHTML=`<bdi dir="ltr">${price(fp)} ${esc(STORE_CONFIG.currency)}</bdi>`;
  }

  if(old){
    old.innerHTML=saving
      ? `<bdi dir="ltr">${price(p.price)} ${esc(STORE_CONFIG.currency)}</bdi>`
      : '';
  ...
  if(save){
    save.innerHTML=saving
      ? `توفير <bdi dir="ltr">${price(saving)} ${esc(STORE_CONFIG.currency)}</bdi>`
      : '';
    save.hidden=!saving;
  }

  if(badge){
    badge.innerHTML=d?`<bdi dir="ltr">-${d}%</bdi>`:'';
    badge.hidden=!d;
  }
...
function renderMainProductImage(){
  ...
  if(!src){
    box.innerHTML='<span style="font-size:72px">✨</span>';
    return;
  }
...
function renderThumbs(){
  ...
  /* ما نعرضوش الصور المصغرة إلا كان أكثر من صورة */
  ...
  box.innerHTML=PRODUCT_IMAGES.map((src,i)=>{
    ...
    return `<button
      type="button"
      class="thumb ${i===currentImageIndex?'active':''}"
...
function renderSimilarProducts(product,key){
  ...
  grid.innerHTML=related.map(p=>{
    ...
    const img=src && (
      ...
      ? `<img src="${attr(src)}" alt="${attr(p.name)}" loading="lazy">`
      : '<span>✨</span>';

    return `<a class="similarCard" href="${productUrl(p.id)}">
      <div class="similarImage">${img}</div>
      <div class="similarBody">
        <small>${esc(normalizeCategory(p.category))}</small>
        <h3>${esc(p.name)}</h3>
        <strong><bdi dir="ltr">${price(fp)} ${esc(STORE_CONFIG.currency)}</bdi></strong>
      </div>
    </a>`;
  }).join('');
}
...
function backToCategory(){
  if(!STORE_CURRENT_PRODUCT){
    location.href='/';
    return;
  }

  const key=categoryKeyFromName(STORE_CURRENT_PRODUCT.category);
  location.href='/category?cat='+encodeURIComponent(key);
}

async function shareProduct(){
  const data={
    title:STORE_CURRENT_PRODUCT?.name||'StoreRi7a',
    text:'شوف هذا المنتج في StoreRi7a',
    url:location.href
  };
...
function addCurrentToCart(){
  if(STORE_CURRENT_PRODUCT){
    add(Number(STORE_CURRENT_PRODUCT.id));
  }
}

function orderCurrentWhatsApp(){
  if(STORE_CURRENT_PRODUCT){
    buySingleProduct(Number(STORE_CURRENT_PRODUCT.id));
  }
}
...
function updateLightboxImage(){
  ...
  if(counter){
    counter.textContent=PRODUCT_IMAGES.length>1
...
function lightboxPrev(){
  ...
  currentImageIndex=
    (currentImageIndex-1+PRODUCT_IMAGES.length)%PRODUCT_IMAGES.length;
...
function applyTransform(){
  ...
  if(label){
    label.textContent=Math.round(lightboxScale*100)+'%';
  }
}
...
function setupLightboxGestures(){
  ...
  stage.addEventListener('pointerdown',e=>{
    ...
    dragStart={
      x:e.clientX-lightboxPanX,
      y:e.clientY-lightboxPanY
    };
  ...
  /* ضغطتين: تكبير 2x، وضغطتين مرة أخرى: رجوع 1x */
  ...
  return `<article class="card">
    ...
    <div class="info">
      ...
      <div class="priceLine">
        ...
        ${fp<p.price
          ? `<span class="oldPrice"><bdi dir="ltr">${price(p.price)} ${esc(STORE_CONFIG.currency)}</bdi></span>`
          : ''}
      </div>
        ...
        ? cart.map(x=>`
          <div class="cartItem">
            <div>
              ...
              <small>
                ${x.qty} ×
                <bdi dir="ltr">${price(finalPrice(x))} ${esc(STORE_CONFIG.currency)}</bdi>
              </small>
            </div>
