...
function smartScroll(id,preview=0){
  ...
  // Keep the selected section heading visible directly under the sticky header.
  const gap=12;
  const y=Math.max(0,absoluteTop-headerH-gap);
...
function renderProductSwipeGallery(){
  ...
  // Always start from the main image when opening a product page.
  currentImageIndex=0;
  ...
  if(dots){
    dots.innerHTML=images.length>1
      ? images.map((_,index)=>`
          <button
            class="productSwipeDot ${index===0?'active':''}"
            type="button"
  ...
  // Prevent browser/history scroll restoration from landing on the last photo.
  requestAnimationFrame(()=>{
    gallery.scrollLeft=0;

    requestAnimationFrame(()=>{
      const first=gallery.querySelector('.productSwipeSlide');
      if(first){
...
function setupLightboxGestures(){
  ...
  // Swipe navigation while image is not zoomed.
  ...
  function endPointer(e){
    ...
    if(shouldCheckSwipe){
      ...
      if(absX>=55 && absX>absY*1.2){
        // Finger left = next image, finger right = previous image.
        if(dx<0){
          lightboxNext();
        }else{
          lightboxPrev();
