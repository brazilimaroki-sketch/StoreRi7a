...
async function loadStoreData(){
  ...
  fetch('/api/settings',{headers:{accept:'application/json'}})
    ...
    .catch(e=>console.warn('Settings API:',e));
...
function setupHomeCategoryLinks(){
  const routes={discounts:'/category?cat=discounts',women:'/category?cat=women',men:'/category?cat=men',beauty:'/category?cat=beauty'};
...
function backToCategory(){
  ...
  location.href='/category?cat='+encodeURIComponent(categoryKeyFromName(STORE_CURRENT_PRODUCT.category));
}
...
function updateLightboxImage(){
  ...
  if(counter) counter.textContent=PRODUCT_IMAGES.length>1?`${currentImageIndex+1} / ${PRODUCT_IMAGES.length}`:'';
  resetZoom();
}
...
function zoomIn(){lightboxScale=Math.min(4,lightboxScale+.5);applyTransform();}
function zoomOut(){lightboxScale=Math.max(1,lightboxScale-.5);if(lightboxScale===1){lightboxPanX=0;lightboxPanY=0;}applyTransform();}
function resetZoom(){lightboxScale=1;lightboxPanX=0;lightboxPanY=0;dragStart=null;applyTransform();}
...
function setupLightboxGestures(){
  ...
  stage.addEventListener('touchend',e=>{
    ...
    if(now-lastTap<320){
      e.preventDefault();
      if(lightboxScale>1) resetZoom();
      else{lightboxScale=2;lightboxPanX=0;lightboxPanY=0;applyTransform();}
      lastTap=0;
      return;
    }
    lastTap=now;
...
function updateCart(){
  ...
  if(items){
    items.innerHTML=cart.length?cart.map(x=>`<div class="cartItem"><div><b>${esc(x.name)}</b><br><small>${x.qty} × <bdi dir="ltr">${price(finalPrice(x))} ${esc(STORE_CONFIG.currency)}</bdi></small></div><button onclick="removeItem(${x.id})">حذف</button></div>`).join(''):'<p>السلة فارغة حالياً.</p>';
  }
  if(total){
    const totalValue=cart.reduce((s,x)=>s+finalPrice(x)*x.qty,0);
