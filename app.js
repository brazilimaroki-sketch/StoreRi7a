...
function finalPrice(p){
  return p.sale_price!==null && p.sale_price>=0 && p.sale_price<p.price ? p.sale_price : p.price;
}
  ...
  return `<article class="card">
    ...
    <div class="info">
      ...
      <div class="priceLine">
        ...
        ${fp<p.price ? `<span class="oldPrice">${price(p.price)} ${esc(STORE_CONFIG.currency)}</span>` : ''}
      </div>
...
function updateCart(){
  ...
  if(total){
    total.textContent=
      price(cart.reduce((s,x)=>s+finalPrice(x)*x.qty,0))+
      ' '+
